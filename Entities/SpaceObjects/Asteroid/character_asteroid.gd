...
func _ready() -> void:
	...
	velocity = movement_vector.rotated(rotation) * randf_range(max_velocity/2, max_velocity)
...
func _physics_process(delta: float) -> void:
	if global_position.y >= 403: #3 pixel buffer
		velocity.y = min(abs(velocity.y) * -1, -25)
	elif global_position.y <= -403:
		velocity.y = min(abs(velocity.y), 25)
	...
	if collision != null :
		...
		if collider is Asteroid:
			...
			var average_vel = (velocity + collider.velocity) / 2.0
			velocity = (pos_diff.normalized() * average_vel.length() * (average_mass/mass)) 
			collider.velocity = (-pos_diff.normalized() * average_vel.length() * (average_mass/collider.mass)) 
		
func take_damage(damage: int):
	health -= damage
	update_health_bar()
	if (health <= 0):
		pickup.movement_vector = velocity.normalized()
